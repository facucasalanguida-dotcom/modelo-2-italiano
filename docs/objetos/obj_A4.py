...
embutido 400 x 400 x 250 (a 90 del canto izquierdo, 110 del derecho y 73
del trasero, medido sobre las fotos) con desague O 52; taladro O 33 para grifo en la repisa trasera
...
SENO = 0.400
SENO_X = -A / 2 + 0.090 + SENO / 2        # a 90 del canto izquierdo (la cota 40 del plano es la ceja de 500; fotos: ~90 izq / 110 der)
...
def build():
    ...
    L.sustraer(tab, L.cilindro('taladro grifo', 0.0165, 0.06, (0.180, Y_TRAS - 0.045, Z_TAB - 0.05), segs=32))
    ...
    for sx, nm in ((-1, 'izquierdo'), (1, 'derecho')):
        ...
        for k, y in enumerate((-0.15, 0.15)):
            L.cilindro(f'remache {nm} {k + 1}', 0.004, 0.001, (sx * (A / 2 - 0.040) - (0.001 if sx < 0 else 0.0), y, zf0 + 0.040), eje='X', segs=16, mat=inox_p)
    ...
    for sx, sy in ((-1, -1), (1, -1), (1, 1), (-1, 1)):
        L.sustraer(est, L.caja('estante recorte', PATA + 0.0002, PATA + 0.0002, 0.10, (sx * (A / 2 - 0.040 - PATA / 2), sy * (F / 2 - 0.040 - PATA / 2), Z_EST - 0.03)))

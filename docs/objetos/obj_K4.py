...
asoma el canto); banda negra bajo el canto delantero; peto trasero de 125
sobre la placa con pestana superior ranurada; petos laterales en cuna;
frontal retranqueado 20 mm con ruleta ON/OFF sobre dial blanco, piloto
verde, ruleta de temperatura sobre dial blanco (50-300), piloto ambar, logo
Cleiton y sello TOP QUALITY; tira de pictogramas y CE a la izquierda; cajon
de grasa abajo a la izquierda; cuatro tacos negros de 30.

Confirmado: medidas exteriores, placa 500 x 425, mandos y pegatinas.
Supuesto: reparto de alturas (patas 30, frontal 150, canto 25, peto 125),
trasera y salida del cable.
"""
import math
import os
...
Z_PLACA = 0.205                # cara de la placa
E_PLACA = 0.012
PLACA_W, PLACA_D = 0.500, 0.425
Y_FRENTE, Y_TRAS = -F / 2, F / 2
Y_PANEL = Y_FRENTE + 0.020     # frontal retranqueado
Y_PLACA0 = Y_FRENTE + 0.040    # arranque de la placa tras el labio
E_PETO = 0.015
...
def build():
    ...
    # --- patas y caja de mandos (retranqueada 20 mm por delante)
    for i, (sx, sy) in enumerate(((-1, -1), (1, -1), (1, 1), (-1, 1))):
        L.cilindro(f'pata {i + 1}', 0.0175, H_PATA, (sx * (A / 2 - 0.045), sy * (F / 2 - 0.050), 0), r=0.004, segs=40, mat=negro)
    caja_d = Y_TRAS - Y_PANEL
    L.caja('caja', A, caja_d, Z_CAJA - H_PATA, (0, Y_PANEL + caja_d / 2, H_PATA), r=0.002, segs=3, mat=inox)
    # --- labio delantero (canal de grasa) con banda negra, y marco de la placa
    L.caja('labio', A, Y_PLACA0 - Y_FRENTE, Z_PLACA - Z_CAJA, (0, (Y_FRENTE + Y_PLACA0) / 2, Z_CAJA), r=0.002, segs=3, mat=inox)
    L.caja('banda negra', A + 0.0004, 0.0008, 0.015, (0, Y_FRENTE + 0.0004, Z_CAJA + 0.004), mat=banda, suave=False)
    ...
    L.caja('peto trasero', A, E_PETO, H - Z_PLACA, (0, Y_TRAS - E_PETO / 2, Z_PLACA), r=0.0015, segs=2, mat=inox)
    pest = L.caja('peto pestana', A - 0.030, 0.032, 0.004, (0, Y_TRAS - E_PETO - 0.016, H - 0.004), r=0.0008, segs=2, mat=inox)
    for i in range(20):
        x = -(A - 0.030) / 2 + 0.020 + i * 0.0255
    ...
    for nm, sx in (('izquierdo', -1), ('derecho', 1)):
        x = sx * (A / 2 - E_PETO / 2)
        L.prisma_yz(f'peto lateral {nm}', [(Y_PLACA0, Z_PLACA - 0.001), (Y_TRAS - E_PETO, Z_PLACA - 0.001), (Y_TRAS - E_PETO, H),
    ...
    P.mando_ruleta('mando onoff', (-0.075, Y_PANEL, zc), d=0.045, alto=0.018, mat=negro)
    P.mando_ruleta('mando temperatura', (0.040, Y_PANEL, zc), d=0.045, alto=0.018, mat=negro)
    P.piloto('piloto verde', (-0.020, Y_PANEL, zc + 0.012), d=0.010, color=(0.1, 1.0, 0.2))
    P.piloto('piloto ambar', (0.098, Y_PANEL, zc + 0.012), d=0.010, color=(1.0, 0.45, 0.05))
    ...
    return dict(ignorar=('cable', 'prensaestopas'))

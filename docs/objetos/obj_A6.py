...
cartelas triangulares (210 de alto x 340 de fondo) con labio doblado en la
...
def build():
    ...
    est = L.caja('estante', A, F, E_CANTO, (0, 0, z_est), r=0.002, segs=3, mat=inox)
    L.sustraer(est, L.caja('estante hueco', A - 0.003, F - 0.003, E_CANTO - 0.0015, (0, 0, z_est - 0.001)))
    ...
    for k, x in enumerate((-A / 2 + 0.070, A / 2 - 0.070)):
        ...
        lab = L.caja(f'cartela {k + 1} labio', 0.015, largo, 0.0015, (x + 0.0075, Y_PARED - CART_D / 2, z_est / 2), mat=inox, suave=False)

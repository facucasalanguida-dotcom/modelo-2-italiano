...
Y_COL = 0.045                        # centro de la columna en Y (mitad trasera de la base)
...
def build():
    ...
    L.cilindro('disco giratorio', 0.057, 0.007, (0, 0.030, 0), segs=96, r=0.002, mat=gris)
    L.toro('disco goma', 0.0575, 0.0025, (0, 0.030, 0.0025), segs=96, segs_r=10, mat=goma)
    ...
    z0 = E_BASE + 0.005
    # garganta curva trasera (solo detras de la columna, embebida 2 mm: sin caras coincidentes)
    L.prisma_yz('columna curva', [(Y_COL + COL_E / 2 - 0.002, z0), (Y_COL + COL_E / 2 + 0.030, z0)] +
                [(Y_COL + COL_E / 2 + 0.030 - 0.030 * math.sin(math.radians(90 * i / 8)), z0 + 0.030 - 0.030 * math.cos(math.radians(90 * i / 8))) for i in range(1, 9)] +
                [(Y_COL + COL_E / 2 - 0.002, z0 + 0.035)],
                -COL_W / 2 + 0.006, COL_W / 2 - 0.006, mat=blanco, r=0.0, suave=True)
    ...
    L.sustraer(marco, L.caja('marco ventana', 0.206, 0.010, 0.152, (-0.008, yb - MARCO_E - 0.003, zb + 0.016), r_vert=0.004))
    piezas.append(marco)
    piezas.append(L.caja('tablet', 0.205, 0.007, 0.151, (-0.008, yb - MARCO_E + 0.004, zb + 0.0165), r=0.001, segs=2, mat=L.mat_aluminio('Aluminio tablet', rug=0.3)))
    piezas.append(L.caja('pantalla cristal', 0.204, 0.001, 0.150, (-0.008, yb - MARCO_E + 0.0015, zb + 0.017), mat=vidrio_n, suave=False))
    ruta = os.path.join(L.CALCAS_DIR, 'B3_pantalla.png')
    calca_pantalla(ruta)
    piezas.append(L.calca('B3 pantalla', ruta, 0.198, 0.145, (-0.008, yb - MARCO_E + 0.0007, zb + 0.092), normal='-Y', emision=1.4))
    rutan = os.path.join(L.CALCAS_DIR, 'B3_nfc.png')
    calca_nfc(rutan)
    piezas.append(L.calca('B3 nfc', rutan, 0.016, 0.016, (0.113, yb - MARCO_E - 0.0003, zb + 0.140), normal='-Y'))
    piezas.append(L.caja('ranura chip', 0.004, 0.003, 0.050, (0.113, yb - MARCO_E - 0.0015, zb + 0.025), mat=L.mat_plastico('Ranura oscura', (0.005, 0.005, 0.005), rug=0.9), suave=False))
    ...
    P.cable('cable', (0.0, Y_COL + COL_E / 2, E_BASE + 0.010), largo=0.25, d=0.004)

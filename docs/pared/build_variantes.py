...
p.append(f'<rect x="0" y="{ZY}" width="{W}" height="{H-ZY}" fill="{AZUL}"/>')
paso, listn = 112, 92                         # listones verticales del zocalo
x = (W % paso) / 2
while x < W:
    p.append(f'<rect x="{x:.0f}" y="{ZY}" width="{listn}" height="{H-ZY}" fill="{AZUL2}"/>')
    x += paso
p.append(f'<line x1="0" y1="{ZY-14}" x2="{W}" y2="{ZY-14}" stroke="{LATON}" stroke-width="12"/>')
p.append(f'<line x1="0" y1="{ZY+26}" x2="{W}" y2="{ZY+26}" stroke="{LATON}" stroke-width="4"/>')
# zona crema superior
p.append(f'<path fill="none" stroke="{LATON}" stroke-width="4" '
